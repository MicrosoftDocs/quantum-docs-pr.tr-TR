---
title: 'Standart kitaplıklarda tür dönüştürmeleri :::no-loc(Q#):::'
description: 'Standart kitaplıklarda ortak ve Kullanıcı tanımlı tür dönüştürme işlevleri hakkında bilgi edinin :::no-loc(Q#)::: .'
author: cgranade
...
<span data-ttu-id="53b25-104">:::no-loc(Q#):::**türü kesin belirlenmiş** bir dildir.</span><span class="sxs-lookup"><span data-stu-id="53b25-104">:::no-loc(Q#)::: is a **strongly-typed** language.</span></span>
<span data-ttu-id="53b25-105">Özellikle :::no-loc(Q#)::: ayrı türler arasında örtük olarak atama yapmaz.</span><span class="sxs-lookup"><span data-stu-id="53b25-105">In particular, :::no-loc(Q#)::: does not implicitly cast between distinct types.</span></span> <span data-ttu-id="53b25-106">Örneğin, `1 + 2.0` geçerli bir :::no-loc(Q#)::: ifade değil.</span><span class="sxs-lookup"><span data-stu-id="53b25-106">For instance, `1 + 2.0` is not a valid :::no-loc(Q#)::: expression.</span></span>
<span data-ttu-id="53b25-107">Bunun yerine, :::no-loc(Q#)::: belirli bir türün yeni değerlerini oluşturmak için çeşitli tür dönüştürme işlevleri sağlar.</span><span class="sxs-lookup"><span data-stu-id="53b25-107">Rather, :::no-loc(Q#)::: provides a variety of type conversion functions for constructing new values of a given type.</span></span>
...
<span data-ttu-id="53b25-112">Son olarak, :::no-loc(Q#)::: standart kitaplık, ve gibi birçok kullanıcı tanımlı tür sağlar <xref:Microsoft.Quantum.Math.Complex> <xref:Microsoft.Quantum.Arithmetic.LittleEndian> .</span><span class="sxs-lookup"><span data-stu-id="53b25-112">Finally, the :::no-loc(Q#)::: standard library provides a number of user-defined types such as <xref:Microsoft.Quantum.Math.Complex> and <xref:Microsoft.Quantum.Arithmetic.LittleEndian>.</span></span>
