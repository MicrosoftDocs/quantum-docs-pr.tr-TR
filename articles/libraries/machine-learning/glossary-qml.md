...
QML kitaplığında, hiper parametreleri değiştirmek için en iyi yol, UDT [`TrainingOptions`](xref:microsoft.quantum.machinelearning.trainingoptions)varsayılan değerlerini geçersiz kıllamaktır. Bunu yapmak için, [`DefaultTrainingOptions`](xref:microsoft.quantum.machinelearning.defaulttrainingoptions) işleviyle çağırır ve varsayılan değerleri geçersiz kılmak için işleç `w/` uygular. Örneğin, 100.000 ölçülerini ve 0,01 öğrenme oranını kullanmak için:
 ```qsharp
