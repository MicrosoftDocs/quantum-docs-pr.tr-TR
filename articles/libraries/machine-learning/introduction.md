---
title: Hisse makine öğrenimi kitaplığı
author: alexeib2
...
Hisse ve bilgi işleme, klasik makine öğrenimi sınıflandırıcıları için güçlü bir alternatiftir, özellikle de özellik sayısına kısa bir değer olan hisse hesaplama kaynağı olarak entanglement ve sınıf çıkarımı için hisse ölçümü kullanmayı.
...
Sınıflandırma, belirli veri örneklerinin $\{y_1, y_2, \lnoktalar, y_d\}$ sınıf etiketlerini çıkarmaktır. "Eğitim veri kümesi", önceden atanmış bilinen etiketlere sahip $ \mathcal{D} =\{(x, y)} $ örnekleri koleksiyonudur. Burada $x $ bir veri örneğidir ve $y $ "eğitim etiketi" adı verilen bilinen etikettir.
...
Öğrenime hisse için $U (\teta) $, $ \teta $ parametrelerinin bir vektörü olduğu ve $M $ ile son ölçümü belirten, doğru etiket çıkarımı ortalama olasılığı $ $ \begin{hizalaması} \mathcal{L} (\teta) = \frac{1}{| \mathcal{D} |} \left (\ sum_ {(x, y_1) \in\mathcal{D}} P (M = y_1 | U (\teta) x) + \ sum_ {(x, y_2) \in\mathcal{D}} P (M = y_2 | U (\teta) x) \right) \end{hizalaması} $ $ burada $P (M = y | z) $, $y $ ' in hisse durum $z $ ' i ölçmesi olasılığının bir olasılığıdır.
