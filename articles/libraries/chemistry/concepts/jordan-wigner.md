...
Daha sonra bu $a ^ \ dagger_j \tus{0}_j = \tus{1}_j $ ve $a ^ \ dagger_j \tus{1}_j = $0.
\Begin{hizalaması} a ^ \ dagger_j & = \begin{bmatrix}0 & 0 \\\ 1 & 0 \ End{bmatrix} = \frac{X_j-iY_j}{2}olduğunu doğrulamak kolaydır. \NO\\\\ a_j & = \begin{bmatrix}0 & 1 \\\ 0 & 0 \end{bmatrix} = \frac{X_j + iY_j}{2}, \end{hizalaması}; burada $X _j $ ve $Y _j $, qubit $X $ üzerinde davranan Pauli-$Y $ ve-$j $ işleçleridir.

>[!NOTE]
> Q # ' da $ \ket{0}$ State $Z $ işlecinin + 1 eigenstate değerini temsil eder. Fizik $ \demet{0}$ ' nin bazı alanlarında düşük enerji zemin durumu ve bu nedenle $Z $ işlecinin-1 eigenstate 'i temsil eder. Bu nedenle, bazı formüller popüler belgeler farklılık gösterebilir.

Kimya kitaplığında, dolu olmayan bir döndürme-orbçeyi göstermek için $ \ket{0}$ kullanıyoruz.
...
Ancak, $ $ \left (\frac{X_j-iY_j}{2}\ sağ) \left (\frac{X_k-iY_k}{2}\ right) = \left (\frac{X_k-iY_k}{2}\ right) \left (\frac{X_j-iY_j}{2}\ right).
...
\begin{hizalaması} a ^ \ dagger_1 & = \left (\frac{X-iY}{2}\ sağ) \otimes 1 \otimes 1\\\\ a ^ \ dagger_2 & = Z\otimes\left (\frac{X-iY}{2}\ sağ) \otimes 1 \ otimes 1 \otimes \cnoktalar \otimes 1\\\\ a ^ \ dagger_3 & = Z\otimes Z\otimes \left (\frac{X-iY}{2}\ sağ) \otimes 1 \otimes \cnoktalar \otimes 1,\\\\ & \vnoktalar\\\\ a ^ \ dagger_N & otimes \left (\frac{X-iY}{2}\ right). \label{EQ: JW} \end{hizalaması}
...
\begin{hizalaması} h_ {PP} a_p ^ \hanger a_p & = \ sum_p \frac{h_ {PP}}{2}(1-Z_p)\\\\ h_ {PQ} (a_p ^ \dağılım a_q + a ^ \ dagger_q a_p) & = \frac{h_ {PQ}}{2}\left (\ prod_ {j = q + 1} ^ {p-1} Z_j \ sağ) \left (X_pX_q + Y_pY_q \sağ)\\\\ h_ {pqqp} n_p n_q & = \frac{h_ {pqqp}}{4}\left (1-Z_p-Z_q + Z_pZ_q \right)\\\\ H_ {pqqr} & = \frac{h_ {pqqr}}{2}\left (\ prod_ {j = r + 1} ^ {p-1} Z_j \ sağ) \left (X_pX_r + Y_pY_r \ sağ) \left (\frac{1-Z_q}{2}\ right)\\\\ H_ {pqrs} & = \frac{h_ {pqrs}}{8}\ prod_ {j = s + 1} ^ {r-1} Z_j \ prod_ {k = q + 1} ^ {p-1} Z_k \Big (XXXX-XXYY + Xyxy\nonumarası\\\\ & \qquad\qquad\qquad\qquad\qdörtlü + YıXXY + YXYX-YYXX\nonumber\\\\ & \qquad\qquad\qquad\qquad\qdörtlü + XYYX + YYYY\Big) \end{hizalaması}
