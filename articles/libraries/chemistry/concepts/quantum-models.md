...
Özellikle, $N _e $ elektriler için momenta $p _i $ (üç boyutta) ve toplu $m _E $ ve konum vektörlerini $x _i Hamiltonian işleci \begin{Equation} \hat{H} = \sum\_{i = 1} ^ {N\_e} \frac{\hat{p}\_i ^ 2} {2m\_e} + \frac{1}{2}\sum\_{i\ne j} \frac{e ^ 2} {| \hat{x}\_ı-\hat{x}\_j |}-\sum\_{ı , k} \frac{Z\_ke ^ 2} {| \hat{x}\_i-{y}\_k |} + \frac{1}{2} \ sum_ {k\ne k '} \frac{Z\_kZ\_{k '} e ^ 2} {| y\_k-y\_k ' |}. \label{EQ: parçalı} \end{Equation} momenta Operators $ \hat{p}\_i ^ 2 $, gerçek alanda Laplacıa işleçleri olarak görüntülenebilir (örn. $ \hat{p}\_i ^ 2 =-\partial\_{x\_ı} ^ 2-\partial\_{y\_i} ^ 2-\ kısmi\_{z\_ı} ^ 2 $.
...
\begin{Equation} H\_{i, j} = \int\_{-\infty} ^ \infty\int\_{-\infty} ^ \infıda \psı ^ {\*}\_ı (x\_1) \hat{H} \psi\_j (x\_2) \gg ^ 3x\_1 \gg ^ 3x\_2. \ Label {EQ: discreteHam} \end{Equation}
...
$ $ Yasaklıyor iki elektriğini aynı hisse başında olan Pauli dışlama prensibi, bu yasaların doğrudan bir sonucu, aynı konumun $ \psi (x_1, x_1) \mapsto \psi ' y a yer alan iki elektriğini değiştirmemiz durumunda ıntuklarca olabilir ( x_1, x_1) \ne-\psi (x_1, x_1) $ $ \psı (x_1, x_1) = 0 $ olmadığı.
