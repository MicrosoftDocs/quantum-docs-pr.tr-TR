...
Diğer bir deyişle, bir $n $-qubit kaydını, hisse miktarı tahmini olarak aşamanın ikili temsilini içerecek şekilde hazırlamak yerine, bir *Klasik* aracının bir hisse ölçümün.
...
Bu tanım yerinde olduğunda, yinelemeli aşama tahmininin her adımı, $ \ket{+} $ durumunda [bulunan bir](xref:microsoft.quantum.concepts.matrix-advanced) avılcal, $U (m) $, yani $U (m) \ket{\phi} = e ^ {im\phi} olduğunu varsaydığımız ilk durum olan $ \ket{\phi} $.  
...
Bu klasik çıkarımı çözmek için Q # Canon ' de sunulan istatistiksel algoritmaları açıklamaya devam etmeden önce, bunu bir teorik olarak, Bayeme parametre tahmini gidermek.
...
Özellikle, tek bir adım, şu Unitary olmayan dönüştürmeyi bir durum olarak gerçekleştirir \begin{hizalaması} \sum_j \sqrt{\Pr (\fi\_j)} \ket{\phi\_j} \mapsto \sum\_j\frac {\ sqrt {\ PR (\fi\_j)} \sqrt{\Pr (\text{Result} | \ Fi\_j)} \ket{\phi\_j}} {\sqrt{\Pr (\fi\_j) \sum\_j \Pr (\text{Result} | \fi\_j)}}.
\end{hizalaması} bu işlem birden çok `Result` değer üzerinde yinelendiği için, $ \prod_k\Pr (\text{Result}\_k | \fi\_j) $ değerinin maxhayvan değerleri olmayan eigenstates, katlanarak bastırılır.
...
Bayes ' teoreminin, aşama tahmininizden elde edilen durumun \begin{hizalaması} \frac{\sqrt{\pr (\fi\_j)} biçiminde yazılmasını da önerir. Sqrt{\pr (\Text{Result} | \phi\_j)} \ket{\phi\_j}} {\sqrt{\pr (\phi\_j) \sum\_j \Pr (\text{Result} | \phi\_j)}} = \sum_j \ sqrt{\pr (\phi\_j | \text{Result})} \ket{\fi\_j}.
...
> [!TIP]
> Uygulamada Bayeme aşaması tahmini hakkında daha fazla bilgi için lütfen [**Phasetahmine**](https://github.com/Microsoft/Quantum/tree/master/Samples/src/PhaseEstimation) örnek bölümüne bakın.
...
\end{Equation} yinelemeli aşama tahmini durumunda, \begin{hizalaması} \Pr (\Texttt{dd} | \fi; e, \teta) & = \left | gibi $ \ket{\psı} = R_1 (e [\phi-\teta]) \ket{+} $ şeklindedir. \braket{+ | R_1 (e [\phi-\teta]) | +} \ sağ | ^ 2 \\\\ & = \left | \frac12 \left (\bra{0} + \bra{1} \ sağ) \left (\ayraç{0} + e ^ {i m [\phi-\teta]} \demet{1} \ sağ) \ doğru | ^ 2 \\\\ & = \left | \frac{1 + e ^ {i m [\phi-\teta]}}{2} \ Right | ^ 2 \\\\ & = \cos ^ 2 (m [\phi-\teta]/2) \tag{★} \label{EQ: Phase-EST-olasılık}.
...
Concreary, \begin{Equation} \Pr (\phi | d) = \frac{\Pr (d | \fi) \Pr (\fi)} {\int \Pr (d | \fi) \Pr (\fi) {\mathrm d} \phi} \Pr (\fi), \end{Equation} burada $d \\\{\Texttt{dd}, \texttt{One}\\} $ bir `Result`ve burada $ \Pr (\fi) $ şeklindedir önceki inanılmaz EFS 'yi $ \phi $ hakkında açıklar.
Bu durumda, posterior Distribution $ \Pr (\phi | d) $, inanılmaz EFS 'yi bir sonraki `Result`gözlemmize hemen açıklayacaktır.

Bu yordam sırasında herhangi bir noktada, klasik denetleyici tarafından gösterilen $ \hat{\phi} $ aşamasını \begin{Equation} \hat{\phi} \mathrel{: =} \beklediği [\phi | \Text{Data}] = \int \phi \Pr (\phi | \Text{Data}) {\mathrm d} \fi, \end{Equation} olarak raporlayabilir $ \ {Data} $ metni, elde edilen tüm `Result` değerlerinin tüm kayıtlarını temsil eder.
...
> [!TIP]
> Sağlam aşama tahmini kullanıldığı birçok örnek vardır. Çeşitli fiziksel sistemin zemin durumu enerji düzeyini çıkartarak aşama tahmini için, lütfen [ **H2 simülasyon** örneğine](https://github.com/Microsoft/Quantum/tree/master/Samples/src/H2SimulationCmdLine), [ **simplefon** örneğe](https://github.com/Microsoft/Quantum/tree/master/Samples/src/SimpleIsing)ve [ **Hubbard model** örneğine](https://github.com/Microsoft/Quantum/tree/master/Samples/src/HubbardSimulation)bakın.
...
[Bayeduyma aşaması tahmini](#bayesian-phase-estimation) için ele alınan tam aynı analizin uygulanabilir ve bu daha genel Oracle modeli için olasılık işlevi tam olarak aynıdır: $ $ \pr (\Texttt{sıfırlaması} | \fi; t, \teta) = \cos ^ 2 \ Left (\frac{t [\phi-\teta]} @no__ t_1_ \right).
