...
Yaklaşık QFT, dyadic $Z $-döndürme <xref:microsoft.quantum.primitive.rfrac> işleminin yanı sıra <xref:microsoft.quantum.intrinsic.h> işlemini de gerektirir.
...
$K \ge \log_2 (n) + \log_2 (1/\epsilon) + $3 için bu bilinmektedir. biri $\\bağlanabilir | \operatorname{QFT}-\operatorname{AQFT} \\| < \epsilon $.
...
$ $ \Operatorname{Add}\ket{a}\ket{b} = \ket{a}\ket{a + b} aracılığıyla iki hisse girişi olan bir Ekleyici 'a dönüştürülebilir, $ $n $ \begin{hizalaması} \operatorname{Add} \ket{a} \ket{b} & = \lambda\_{a\_0} \left (\operatorname{Add} (1) \right) \Lambda\_{a\_1} \left (\operatorname{Add} (2) \right) \Lambda\_{a\_2} \left (\operatorname{Add} (4) \right) \cnoktalar \Lambda\_{a\_{n-1}} \left (\ operatorname {Add} ({{n-1}}) \right) \ket{a}\ket{b} \\\\ & = \ket{a} \ket{b + a}, \end{hizalaması} $n $-bit tamsayılar $a $ ve $b $ ve ek modül $2 ^ n $.  $ \Lambda\_x (A) $ öğesinin her türlü işlemi için $A $, qubit $x $ as denetimi ile bu işlemin denetlenen sürümüne olduğunu hatırlayın.
...
$ $ $ $ \Ket{\phi\_k (a)} = \frac{1}{\sqrt{2}} \left (\ket{0} + e ^ {i2\pi a/2 ^ k} \tus{1} \ right) tanımladığımızda, $ $ daha sonra bazı algeköşeli sonrasında $ $ \operatorname{QFT}\ket{a} = \ket{\phi\_1 (a)} \otimes \cnoktalar \ ' a bakabilirsiniz. otimes \ket{\phi\_n (a)}.
...
### <a name="quantum-phase-estimation"></a>Hisse aşamalı tahmin ###
...
$V $ öğesinin denetlenen bir $U $ uygulaması olduğunu varsayalım. bu nedenle, \begin{hizalaması} V (\demet{0} \otimes \ket{\phi}) & = \demet{0} \otimes \ket{\phi} \\\\ \textrm{ve} V (\demet{1} \otimes \ket{\phi}) & = e ^ {ı \ Phi} \tus{1}  \otimes \ket{\phi}.
...
\end{hizalaması} Bu \ Begin{hizalaması} V (\ket{+} \otimes \ket{\phi}) öğesini bulmak için şartlar toplayabiliriz & = \frac{\tus{0} + e ^ {ı \phi} \ket{1}} {\sqrt{2}} \otimes \ket{\phi} \\\\ & = (R_1 (\fi) \ket{+}) \otimes \ket{\phi} , \end{hizalaması}; burada $R _1 $, <xref:microsoft.quantum.intrinsic.r1> işlemi tarafından uygulanan Unitary 'dir.
...
Bu şekilde devam ederseniz, \ Begin{hizalaması} \ket{\psı} & = \sum_{j = 0} ^ n R_1 (2 ^ j \ Fi) \ket{+} biçiminde kayıt elde ede\\biliriz \\ & \propto \bigotimes_{j = 0} ^ {n} \left (\demet{0} + \exp (i 2 ^ {j} \fi) \tus{1}\right) @no__ \\ t_4_ & \sum_{k = 0} ^ {2 ^ n-1} \exp (i \phi k) \ket{k} \end{hizalaması}; burada $n $, gereken duyarlık bitlerinin sayısı ve ${} \propto {}$ 'ın normalleştirme faktörünü gizliyoruz 1/\ sqrt{2 ^ n} $.

$ \Phi = 2 \ Pi p/2 ^ k $ ' ı bir tamsayı $p $ için kabul etmemiz durumunda bunu $ \ket{\psı} = \operatorname{QFT} \ket{p_0 p_1 \noktalar p_n} $ olarak tanıyacağız; burada $p _j $, $j ^ {\textrm{TH}} $ bit/$2 \pi \phi $ şeklindedir.
