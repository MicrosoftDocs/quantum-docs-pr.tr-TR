...
Koşullu ifade, formun bir ifadesidir `condition ? caseTrue | caseFalse` .
Yukarıdaki örnekte görünebileceğini de, diziler üzerinde yineleme de desteklenir.

Ve içindeki `using` ayırmalar `borrowing` , tek qubits, qubit dizileri ve iç içe geçmiş tanımlama alanları için yeni bir olanaklardır:
```qsharp
    using (qubit = Qubit()) {
...
İki yeni özelleştirme Oluşturucu yönergesi `invert` ve `distribute` buna ek olarak `auto` , functor uzmanlıklarıyla, denetimin nasıl üretilecektir.
Varsayılan `body` özelleştirmenin işlemler içindeki açık bildirimi artık zorunlu değildir.
...
Değiştiriciler, diğer herhangi bir ifadeden daha sıkı bir şekilde bağlanır ve çağrı ifadesi Combinator ' den daha sıkı Combinator `(` `)` . 
`Adjoint`, `Controlled` ve yeni bir "sarmalama" sonek işleci `!` ifade değiştiricileri olarak değerlendirilir. `Adjoint` `Controlled` İfade değiştiricilerini düşünerek, çok sayıda durum için parantez gereksinimini ortadan kaldırır. Örneğin `Op` , bir Unitary işlemi, `opArr` Unitary işlemleri dizisi ve `arg` uygun bir bağımsız değişken için aşağıdaki ifadeler geçerlidir: 
```qsharp
    Adjoint Op (arg);
    Controlled opArr[i] (arg);
```
Geçiş betiği, gereksiz parantezleri büyük bir kısmını bırakacak ve yeni özelliklerden tam olarak yararlanacaktır. Ancak, betiğin kodu yorumlamaması ve `Controlled` nadir bir durumda (yalnızca kısmi uygulamalarla birlikte ilgili olarak geçerlidir) farklı durumlarda functor farklı yorumlamasına yönelik hesaba sahip olmaması önerilir.
...
```
Örnekte görüldüğü gibi, `!` ve gibi önek değiştiricilerinden daha sıkı bir şekilde bağlanır `Controlled` `Adjoint` .

## <a name="breaking-changes"></a><a name="BreakingChanges"></a>Son değişiklikler 
...
Aşağıda listelenen değişikliklere ek olarak, `__*__` * öğesinin herhangi bir boşluk olmayan karakter dizisi iç kullanım için ayrılmıştır ve sembol adı olarak kullanılamaz.  
...
Böyle bir değişiklik için bir örnek, Kullanıcı tanımlı uzmanlıkdaki sembol tanımlama grubu ' dir. Önceki sürümlerde denetlenen veya denetlenen-adjoint özelleşme bildirimi tek bir symbol bağımsız değişkeni belirtirken, 0,3 ' den başlayarak, `...` derleyicinin sembolleri, gerekirse çağrılabilir bildiriminden geçirdiğine işaret eden tüm özelleşmeye yönelik bir sembol tanımlama grubu sağlaması önerilir. 
...
Aşağıdaki örnek, `ToOperation` bir işlevi bir işleme değiştirmek için işlevinin nasıl kullanılacağını gösterir `Square` `op` : 
```qsharp
...
## <a name="migrating-existing-projects-to-q-03"></a><a name="Migration"></a>Mevcut projeleri Q # 0,3 ' a geçirme 
...
2. Bir metin düzenleyicisinde, "Microsoft. hisse" öğesinin tüm sürümünü sürüm 0.3.1811.2802-Preview sürümüne değiştirmek için. csproj dosyasını düzenleyin `PackageReference` , örneğin:
```xml
...
4. Komut satırından, bu komutu çalıştırarak tüm sözdizimi değişikliklerini karşılamak için komut satırı derleyicisi ile tümleştirilmiş biçimlendirme aracını kullanın:`dotnet msbuild /t:qsharpformat`  
    - Dosyalarınız yerinde geçirilecek. Tüm özgün dosyaların bir yedeklemesi, ' ye kopyalanacak`obj\qsharp\.backup`
...
## <a name="migration-script"></a><a name="MigrationScript"></a>Geçiş betiği 
...
- Bu proje, geçişin başarılı olma olasılığını sağlamak için hisse geliştirme setinin önceki sürümüyle oluşturulmuştur. Bu adım, `-Force` komut dosyasının parametresi kullanılarak atlanabilir.
...
> Windows 'da çalıştırmak gibi betiklerin çalıştırılmasına izin vermek için `qdk-migrate.ps1` , yürütme ilkesini değiştirmeniz gerekebilir.
