...
Norm $1 $ ile gerçek veya karmaşık sayıların herhangi bir iki boyutlu sütun vektörü, bir qubit tarafından tutulan olası bir hisse TI durumunu temsil eder. Bu nedenle $ \begin{bmatrix} \Alpha \\ \\ \beta \end{bmatrix} $ bir qubit durumu temsil eder $ \Alpha $ ve $ \beta $, $ | \Alpha | ^ 2 + | \beta | ^ 2 = $1 ' i karşılarken karmaşık sayılardır. Qubits 'i temsil eden geçerli hisse alma durumu vektörlerine örnek olarak şunlar verilebilir
...
Bir qubit 'i nasıl temsil ettiğini öğrendiğimiz için, [*ölçüm*](https://en.wikipedia.org/wiki/Measurement_in_quantum_mechanics)kavramını tartışarak bu durumların neleri temsil ettiğini öğrenmek için bazı ıntutik kazanıyoruz. Bir ölçü, bir qubit 'de "bakıma" gibi, her iki klasik durumdan biri olan $ \begin{bmatrix} 1 \\ \\ 0 \end{bmatrix} $ veya $ \begin{bmatrix} $ veya $ \ Begin{bmatrix} $ veya $ \ Begin{bmatrix} \\ \\ $. Hisse durumu vektörü $ \begin{bmatrix} \Alpha \\ \\ \ Beta \end{bmatrix} $ tarafından verilen bir qubit ölçüldüğünde, $0 $ sonucunu olasılık $ | \Alpha | ^ 2 $ ve sonuç $1 $ ile olasılık $ | \beta | ^ 2 $ ile elde ediyoruz. Sonuç $0 $, qubit 'in yeni durumu $ \begin{bmatrix} 1 \\ \\ 0 \end{bmatrix} $; Outcome. Sonuç $1 $, durumu $ \begin{bmatrix} 0 \\ \\ 1 \ End{bmatrix} $ şeklindedir. Bu olasılıkların, $ | \Alpha | ^ 2 + | \beta | ^ 2 = $1 normalleştirme koşulu nedeniyle $1 $ ' e kadar olduğunu unutmayın.

Ölçümün özellikleri, hisse maeyalet vektörünün genel işaretinin ilgisiz olduğunu da ifade demektir. Bir vektörü negatifi, $ \Alpha \sağtarrow-\Alpha $ ve $ \beta \ tam Tarrow-\beta $ ile eşdeğerdir. $0 $ ve $1 $ ölçmesinin olasılığı koşulların büyüklüğüne bağlı olduğundan, bu tür bir işaret eklemek, olasılıkların koşullarını değiştirmez. Bu tür aşamalar genellikle [ `` *genel aşamalar*' '](https://en.wikipedia.org/wiki/Phase_factor) olarak adlandırılır ve daha fazla genel olarak, yalnızca $ \pm $1 yerine ^ {ı \phi} $ $e biçiminde olabilir.

Ölçümün son önemli özelliği, tüm hisse maeyalet vektörlerine zarar vermeyedir. Klasik durum $0 $ ' e karşılık gelen $ \begin{bmatrix} 1 \\ \\ 0 \end{bmatrix} $ durumunda bir qubit ile başladığımızda, bu durumun ölçülmesi her zaman $0 $ sonucunu verir ve hisse durumu değişmeden bırakır. Bu anlamda, yalnızca klasik bitlerin (yani, $ \begin{bmatrix}1 \\ \\ 0 \end{bmatrix} $ veya $ \ Begin{bmatrix} $ veya $ \begin{bmatrix} $ veya $ \ Begin{bmatrix} $) sahip olduğu ve \\ \\ ölçüm sisteme zarar vermemiz durumunda. Bu, klasik verileri çoğaltabilmemiz ve bir hisse bir bilgisayar üzerinde yalnızca klasik bir bilgisayarda olabileceği gibi işleyebilmemiz anlamına gelir. Ancak, bilgileri bir kerede her iki durumda da depolamak için, uygun şekilde ne kadar iyi bir şekilde veri depolama alanı ve diğer Robs hisse [bilgisayarları da sayısının fark gözetmeden](https://en.wikipedia.org/wiki/No-cloning_theorem)
...
Bu, önemsiz bir örnek olsa da ( <xref:microsoft.quantum.intrinsic.h> işlem kendi kendine Joint olduğu için), bunun daha karmaşık qubit işlemler için nasıl değerli hale geldiğini görebilirsiniz.
...
Diğer bir deyişle, herhangi bir Unitary dönüştürmesi bu kümeden bir kapı ürünü olarak yaklaşık olarak yazılmışsa, bir ağ geçidi kümesi evrensel bir kapı kümesidir. Herhangi bir önceden tanımlanmış hata varsa, {1} ağ geçidi kümesinden $G _, g_ {2} , \lnoktalar, G_N $ gibi var olan kapıları

$ $ G_N G_ {N-1} \cnoktalar G_2 G_1 \yaklaşık U. $ $

Matris çarpma kuralı, bu dizideki ilk kapı işleminin sağdan sola çarpılacağı, $G _N $, gerçekten de hisse senedi durumu vektörüne uygulanan en son bir şeydir. Daha basit bir deyişle, bu tür bir kapı kümesi, her hata toleransı $ \epsilon>$G $0 G_N $G _N \lnoktalar G_1 $ ve $U $ arasındaki uzaklığın en fazla $ \epsilon $ olduğunu varsayalım. İdeal olarak $ \epsilon $ ' in bu uzaklığına ulaşmak için gereken $N $ değeri, $1/\ Epsilon $ ile Poly-logaritmayı büyük ölçüde ölçeklendirmelidir.
...
$ $ H = \frac {1} {\sqrt {2} } \begin{bmatrix} 1 & 1 \\ \\ 1 &-1 \end{bmatrix}, \qdörtlü S = \begin{bmatrix} 1 & 0 \\ \\ 0 & ı \end{bmatrix} = T ^ 2, \qdörtlü X = \begin{bmatrix} 0 &1 \\ \\ 1& 0 \end{bmatrix} = HT ^ 4h, $ $
...
Unitary dönüştürmelerinin bu temel öğelerden nasıl derlenebilir bir örnek olarak, yukarıdaki Bloch son 'da bulunan üç dönüşüm, $ \begin{bmatrix} 1 \\ \\ 0 \ End{bmatrix} \mapsto HZH \begin{bmatrix} 1 \\ \\ 0 \ End{bmatrix} = \begin{bmatrix} 0 \\ \\ 1 \end{bmatrix} $ öğesine karşılık gelir.

Önceki en popüler temel noktaları, yığının mantıksal düzeyinde (mantıksal düzeyin hisse algoritması düzeyi olarak düşünün) tanımlamak için en popüler temel noktaları oluştursa da, bir işlev açıklaması düzeyine daha yakın işlemler gibi, algoritmik düzeyinde daha az temel işlemleri düşünmek çok uygundur. Neyse ki, Q # Ayrıca üst düzey birimlere uygulama için kullanılabilir yöntemlere sahiptir. Bu, üst düzey algoritmaların, her şeyin Clienfford ve $T $-Gates ' e kadar açık bir şekilde çıkarılması gerekmeden uygulanmasına olanak tanır.
