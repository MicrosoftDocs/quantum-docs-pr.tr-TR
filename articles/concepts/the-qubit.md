...
Ölçümün son önemli özelliği, tüm hisse maeyalet vektörlerine zarar vermeyedir. Klasik durum $0 $ ' e karşılık gelen $ \begin{bmatrix} 1 \\\\ 0 \end{bmatrix} $ durumunda bir qubit ile başladığımızda, bu durumun ölçülmesi her zaman $0 $ sonucunu verir ve hisse durumu değişmeden bırakır. Bu anlamda, yalnızca klasik bitlerin (örneğin, $ \begin{bmatrix}1 \\\\ 0 \end{bmatrix} $ veya $ \begin{bmatrix}0 \\\\ 1 \end{bmatrix} $) sahip olduğu gibi, ölçüm sisteme zarar vermez. Bu, klasik verileri çoğaltabilmemiz ve bir hisse bir bilgisayar üzerinde yalnızca klasik bir bilgisayarda olabileceği gibi işleyebilmemiz anlamına gelir. Ancak, bilgileri bir kerede her iki durumda da depolamak için, uygun bir şekilde ve ayrıca, hisse verileri sayısının fark gözetmeden kopyalama özelliğine göre daha fazla Robs bilgi [ ](https://en.wikipedia.org/wiki/No-cloning_theorem).
...
Matris çarpma kuralı, bu dizideki ilk kapı işleminin sağdan sola çarpılacağı, $G _N $, gerçekten de hisse senedi durumu vektörüne uygulanan son bir şeydir. Daha da benzer bir şekilde, bu tür bir kapı kümesi, her hata toleransı $ \epsilon > 0 $G $ varsa, $G _N \ lnoktalar G_1 $ ve $U $ arasındaki mesafe en fazla $ \epsilon $ olduğunda, bu tür bir ağ geçidi kümesinin Universal olduğunu varsayalım. İdeal olarak $ \epsilon $ ' in bu uzaklığına ulaşmak için gereken $N $ değeri, $1/\ Epsilon $ ile Poly-logaritmayı büyük ölçüde ölçeklendirmelidir.
...
Unitary dönüştürmelerinin bu temel öğelerden nasıl derlenildiği hakkında bir örnek olarak, yukarıdaki Bloo son 'da bulunan üç dönüşümler $ \begin{bmatrix} 1 \\\\ 0 \end{bmatrix} \mapsto HZH \begin{bmatrix} 1 öğesine karşılık gelir \\\\ 0 \end{bmatrix} = \begin{bmatrix} 0 \\\\ 1 \end{bmatrix} $.
...
En basit bu ilkel değer tek qubit Dönüştür. Üç adet tek qubit döndürmeler genellikle kabul edilir: $R _X $, $R _Y $ ve $R _Z $. Örneğin, $R _X (\teta) $ döndürme eylemini görselleştirmek için, sağ Thumb 'ı Bloch Sphere 'in $x $-ekseninin yönü boyunca gösterme ve vektörü bir $ \ teta/2 $ radyanıyla döndürme. Bu kafa karıştırıcı $2 $, şişenin şişine bağlı olarak, k180 ^ \circ $ olduğunu bulmadan, ancak gerçekten de geometrik olarak $90 ^ \circ Karşılık gelen Unitary matrisleri şunlardır:

\begin{align *} & R_z (\teta) = e ^ {-ı\teta z/2} = \begin{bmatrix} e ^ {-i \ teta/2} & 0\\\\ 0 & e ^ {ı \ teta/2} \end{bmatrix}, \\\\ & R_x (\teta) = e ^ {-ı\teta x/2} = HR_z (\teta) H = \begin{bmatrix} \cos (\ Teta/2) &-ı\sin (\ teta/2)\\\\-ı\sin (\ teta/2) & \cos (\ teta/2) \end{bmatrix}, \\\\ & R_y (\teta) = e ^ {-i\teta Y/2} = SHR_z (\teta) HS ^ \hanger = \begin{bmatrix} \cos (\ teta/2) &-\sin (\ teta/2) @no__ t_9_ \\ \sin (\ teta/2) & \cos (\ teta/2) \end{bmatrix}. \end{align*}

Üç boyutta de rastgele bir döndürme gerçekleştirmek için üç döndürmeler birlikte birleştirilebileceği gibi, blok Sphere gösteriminden herhangi bir Unitary matrisinin üç döndürmeler sırası olarak yazılabileceğini de görebilirsiniz. Özellikle, her Unitary matrisi için $U $ \Alpha, \beta, \gama, \delta $ gibi $U = e ^ {i\alpha} R_x (\beta) R_z (\gama) R_x (\delta) $ bulunur. Bu $R nedenle, _Z (\teta) $ ve $H $, farklı bir küme olmasa da bir Evrensel kapı kümesi oluşturur, çünkü $ \teta $ herhangi bir değer alabilir. Bu nedenle ve hisse uygun bir şekilde, bu tür sürekli kapıların, bu nedenle hisse için, özellikle de hisse algoritması tasarım düzeyinde, bu tür sürekli kapılarda önemli olması için Hataya dayanıklı donanım uygulamalarına ulaşmak için, sonunda bu döndürmeleri yakın olan ayrık geçit dizileri olarak derlenir.
