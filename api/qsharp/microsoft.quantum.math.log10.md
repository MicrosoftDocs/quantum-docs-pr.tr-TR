---
uid: Microsoft.Quantum.Math.Log10
title: Log10 işlevi
ms.date: 10/26/2020 12:00:00 AM
ms.topic: article
qsharp.kind: function
qsharp.namespace: Microsoft.Quantum.Math
qsharp.name: Log10
qsharp.summary: Returns the base 10 logarithm of a specified number.
ms.openlocfilehash: 6ffdc19ac74e132ade80e2754a7060816b044c2a
ms.sourcegitcommit: 29e0d88a30e4166fa580132124b0eb57e1f0e986
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 10/27/2020
ms.locfileid: "92726600"
---
# <a name="log10-function"></a>Log10 işlevi

Ad alanı: [Microsoft. hisse. Math](xref:Microsoft.Quantum.Math)

Leyebilir [](https://nuget.org/packages/)


Belirtilen sayının 10 tabanında logaritmasını döndürür.

```qsharp
function Log10 (input : Double) : Double
```


## <a name="input"></a>Giriş

### <a name="input--double"></a>Giriş: [Double](xref:microsoft.quantum.lang-ref.double)





## <a name="output--double"></a>Çıkış: [Double](xref:microsoft.quantum.lang-ref.double)



## <a name="remarks"></a>Açıklamalar

Daha fazla ayrıntı için bkz. [System. Math. Log10](https://docs.microsoft.com/dotnet/api/system.math.log10) .